...
@Configuration
@EnableAsync
public class WallaAsyncConfig implements AsyncConfigurer {
    private static final Logger logger = LoggerFactory.getLogger(GmailAsyncConfig.class);
    @Bean (name = "wallaTaskExecutor")
    public ThreadPoolTaskExecutor wallaTaskExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(3);
        executor.setMaxPoolSize(5);
        executor.setQueueCapacity(20);
        executor.setThreadNamePrefix("WallaThread-");
        executor.setRejectedExecutionHandler((r, executor1) -> {
            logger.info("❌ Task was rejected. Reason: Walla thread pool exhausted.");
    ...
    public @NonNull AsyncUncaughtExceptionHandler getAsyncUncaughtExceptionHandler() {
        return (throwable, method, args) -> {
            System.err.println("🔴 Async exception in Walla method: " + method.getName());
